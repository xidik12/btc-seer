    ...
    <div className="space-y-3" ref={cardRef}>
      ...
      <div className="bg-bg-card rounded-2xl p-4 border border-white/5">
        <div className="flex items-center justify-between mb-2">
          <div />
          <CardShareButton cardRef={cardRef} label="BTC vs Gold" filename="btc-vs-gold.png" />
        </div>
        <div className="flex items-center justify-between mb-3">
          <div
            className={calcs.ratio ? 'cursor-pointer hover:opacity-80 active:scale-[0.98] transition-all' : ''}
            onClick={() => showCalc('ratio', t('market:powerLaw.gold.btcInGold'))}
          >
            <div className="text-text-muted text-[10px] flex items-center gap-1">
              {t('market:powerLaw.gold.btcInGold')}
              {calcs.ratio && (
          ...
          <div className="grid grid-cols-2 gap-2">
            {Object.entries(data.projections).map(([key, val]) => (
              <div key={key} className="bg-bg-card rounded-xl p-3 border border-white/5 text-center">
                ...
                <div className="text-[#ffd700] text-sm font-bold">{safeFixed(val, 1)} oz</div>
              </div>
            ))}
          </div>
        </div>
